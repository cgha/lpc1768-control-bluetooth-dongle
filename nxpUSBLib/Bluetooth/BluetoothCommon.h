...
#define _BLUETOOTH_COMMON_H_
	...
	/* Defines: */
		...
		#define BT_MAX_DEVICE_CONNECTIONS         1
		...
		#define BT_MAX_QUEUED_L2CAP_EVENTS        5
	...
	/* Enums: */
		...
		enum BT_LocalAuthenticationRequire_t
		{
			MITMNotRequire_NoBonding = 0,
			MITMRequire_NoBonding = 1,
			MITMNotRequire_DedicatedBonding = 2,
			MITMRequire_DedicatedBonding = 3,
			MITMNotRequire_GeneralBonding = 4,
			MITMRequire_GeneralBonding = 5,
		};
...
//#pragma pack(1)			//设定为1字节对齐
		...
		typedef struct
		{
			uint8_t  RemoteBDADDR[6]; /**< Remote Bluetooth device's BDADDR for the connection. */
		...
		}ATTR_PACKED BT_L2CAP_Channel_t;
		...
		typedef struct
		{
			...
			struct
			{
				struct
				{
					uint8_t             LocalBDADDR[6]; /**< Address of the local Bluetooth adapter attached to this stack instance. */

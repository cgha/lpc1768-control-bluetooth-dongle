 ...
 */
static const uint8_t RFCOMM_CRC8Table[256] =
	{
...
static uint8_t CRCLength;
static bool CommandResp;
static uint16_t len;
static bool ret;
static uint8_t * ptr;
...
static RFCOMM_Channel_t* RFCOMM_FindChannel(BT_StackConfig_t* const StackState,
...
static RFCOMM_Channel_t* RFCOMM_NewChannel(BT_StackConfig_t* const StackState,
                                                 BT_L2CAP_Channel_t* const ACLChannel,
                                                 const uint8_t DLCI)
{
...
bool RFCOMM_SendFrame(BT_StackConfig_t* const StackState,
                             BT_L2CAP_Channel_t* const ACLChannel,
                             const uint8_t DLCI,
                             uint8_t Control,
                             ...
                             const void* Data)
{
	...
	*ptr = (DataLen << 1);
	...
	CRCLength = sizeof(ResponsePacket.FrameHeader);
	...
	*ptr = RFCOMM_ComputeFCS(&ResponsePacket, CRCLength);
	ptr++;
	len = ptr- (uint8_t*)(&ResponsePacket);
	//ResponsePacket.FCS = RFCOMM_ComputeFCS(&ResponsePacket, CRCLength);
	/* Send the completed response packet to the sender */

	ret = Bluetooth_L2CAP_SendPacket(StackState, ACLChannel, len, &ResponsePacket);
	return ret;
                                    ...
                                    const void* Data)
{
	struct
	{
		...
		uint8_t          Length;
		uint8_t          Data[DataLen];//since here no other variable defined,will not mix with other new defined varible
	} ATTR_PACKED ResponsePacket;

	len = sizeof(RFCOMM_Command_t)+sizeof(ResponsePacket.Length)+DataLen;
	...
	//return RFCOMM_SendFrame(StackState, ACLChannel, RFCOMM_CONTROL_DLCI, RFCOMM_Frame_UIH, len, &ResponsePacket);
	ret = RFCOMM_SendFrame(StackState, ACLChannel, RFCOMM_CONTROL_DLCI, RFCOMM_Frame_UIH, len, &ResponsePacket);
	return ret;
                                    ...
                                    const uint16_t CommandDataLen)
{
	...
	if (CommandHeader->CR)
	{
		...
		printf("\r\n%x",RFCOMMChannel->DataLink.RemoteSignals);
